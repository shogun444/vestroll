...
export default function CompanyInfoPage() {
    ...
    OrganizationApi.getProfile()
      .then((profile) => {
        setFormData((prev) => ({
          ...
          country: profile.registered.country ?? "",
        }));
      })
      .catch(() => {
        // Non-fatal: form stays empty if fetch fails
      })
  ...
  const isFormValid = !!(formData.companyName && formData.registeredName);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg("Company information saved successfully.");
    } catch (err) {
